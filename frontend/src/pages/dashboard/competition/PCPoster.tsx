...
export const PenComPoster: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        setLoading(true)
        console.log("[v0] Fetching poster competition data...")
        const response = await adminAPI.getCompetition(token, "POSTER")
        ...
        if (response && typeof response === "object") {
          ...
          console.log("[v0] Poster competition data:", responseData)
        ...
        const errorMsg = err instanceof Error ? err.message : "Gagal memuat data kompetisi"
        console.error("[v0] Error fetching poster competition:", errorMsg)
        setError(errorMsg)
      } finally {
        setLoading(false)
      }
  ...
  const filteredData =
    activeTab === "individu"
      ? individuData.filter(
          (individu) =>
            individu.fullName.toLowerCase().includes(searchTerm.toLowerCase()) ||
            (individu.school?.toLowerCase().includes(searchTerm.toLowerCase()) ?? false) ||
            individu.leaderEmail.toLowerCase().includes(searchTerm.toLowerCase()) ||
            individu.leaderPhone.toLowerCase().includes(searchTerm.toLowerCase()),
        )
      : timData.filter(
          (tim) =>
            tim.teamName.toLowerCase().includes(searchTerm.toLowerCase()) ||
            tim.leaderName.toLowerCase().includes(searchTerm.toLowerCase()) ||
            (tim.school?.toLowerCase().includes(searchTerm.toLowerCase()) ?? false) ||
  ...
  return (
    <div className="p-6 w-full">
      <h1 className="text-3xl font-semibold text-gray-800 mb-6">Daftar Peserta Competition Poster</h1>
      ...
      <div className="flex justify-between items-center border-b border-gray-300 mb-6">
        <div className="flex space-x-4">
          ...
          <button
            ...
            className={`px-6 py-2 font-medium text-sm transition-colors ${
              activeTab === "tim" ? "border-b-2 border-blue-500 text-blue-600" : "text-gray-500 hover:text-blue-500"
            }`}
          >
            Team
          </button>
          ...
          <thead className="bg-gray-100 text-gray-700 uppercase text-xs">
            {activeTab === "individu" ? (
              <tr>
                ...
                <th className="px-4 py-3 border-b text-left">Metode Pembayaran</th>
                <th className="px-4 py-3 border-b text-left">Kartu Siswa</th>
                <th className="px-4 py-3 border-b text-left">Bukti Pembayaran</th>
                <th className="px-4 py-3 border-b text-left">Bukti Follow IG</th>
              </tr>
            ) : (
              <tr>
                <th className="px-4 py-3 border-b text-left">No</th>
              ...
              activeTab === "individu" ? (
                (filteredData as typeof individuData).map((individu, index) => (
                  <tr key={individu.id} className="hover:bg-gray-50 border-b last:border-none">
                    ...
                    <td className="px-4 py-2">{renderFileLink(individu.memberCard)}</td>
                    <td className="px-4 py-2">{renderFileLink(individu.paymentUrl)}</td>
                    <td className="px-4 py-2">{renderFileLink(individu.igFollow)}</td>
                  </tr>
                ))
              ) : (
                (filteredData as typeof timData).map((tim, index) => (
                  <tr key={tim.id} className="hover:bg-gray-50 border-b last:border-none">
                    ...
                    <td className="px-4 py-2">{renderFileLink(tim.memberCard)}</td>
                    <td className="px-4 py-2">{renderFileLink(tim.paymentUrl)}</td>
                    <td className="px-4 py-2">{renderFileLink(tim.igFollow)}</td>
                  </tr>
                ))
              )
            ) : (
              <tr>
                <td colSpan={activeTab === "tim" ? 11 : 10} className="text-center py-6 text-gray-500">
