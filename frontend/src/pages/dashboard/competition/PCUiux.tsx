...
export const PenComUiUx: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        setLoading(true);
        console.log("[v0] Fetching UI/UX competition data...");
        const response = await adminAPI.getCompetition(token, "UIUX");
        ...
        if (response && typeof response === "object") {
          ...
          console.log("[v0] UI/UX competition data:", responseData);
        ...
        console.error("[v0] Error fetching UI/UX competition:", errorMsg);
        setError(errorMsg);
      } finally {
        setLoading(false);
      }
